...
echo "Human"
	Rscript --vanilla /media/Helios_scStorage/Ralf/Benchmark/DavidRuns/Adjust_gene_names_both_organism.R ${infile} ${outfile} ${method} ${organism}
	awk '{print $1 "\t" $3}' ${infile} > ${outfile}

fi
if [[ ${organism} == "mouse" ]]; then
	echo "Mouse"
	Rscript --vanilla /media/Helios_scStorage/Ralf/MouseHom_AMI/Results/Kallisto/Adjust_gene_names.R ${infile} ${outfile} ${method}
fi
